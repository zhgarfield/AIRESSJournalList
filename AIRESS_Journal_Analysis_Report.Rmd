...
- Construction of a **within-subcategory impact metric** using Principal Component Analysis (PCA) on CiteScore, SNIP, and SJR.
...
- Rescaling of impact scores to a **0–100 Impact Index**, computed separately within each subcategory.
- Application of **k-means clustering (k = 4)** within each subcategory to identify natural journal tiers:
  - **A: Elite**
  - **B: Excellent**
  - **C: Preferred**
  - **D: Acceptable**
- Generation of diagnostic visualizations (loadings, distributions, ECDFs) to assess the stability and clarity of tier separation.
...
| **Political Science and IR**          | Political Science and International Relations; Public Administration; Religious Studies                                                                                                      |
...
| **Other**                             | Social Sciences (Miscellaneous); General Social Sciences; General Agriculture and Biological Sciences; General Medicine; Communication; Philosophy                                            |
                                      |
...
```{r index-hist, fig.cap="The Impact Index (0–100) is derived from the first principal component (PC1) of CiteScore, SNIP, and SJR. Y-axes are scaled independently within each subcategory to emphasize the internal structure of impact distributions rather than differences in the number of journals across fields. The distribution is highly right-skewed, with most journals concentrated at the lower end of the impact spectrum and a long tail of increasingly influential outlets. This pattern reflects the typical unevenness of citation-based metrics, where a small subset of journals capture disproportionately high scholarly visibility and influence.", fig.width=13, fig.height=7}
...
```{r clusterplot, fig.cap="K-means cluster centers by subcategory. The figure shows the mean Impact Index for each cluster within disciplinary subcategories. Clusters are ordered post hoc by mean impact, and tier labels (Excellent, Preferred, Acceptable) reflect relative standing rather than the numeric cluster identifiers produced by k-means.", fig.width=12, fig.height=7}
...
```{r ecdfplot, fig.cap="ECDFs of the Impact Index by cluster and subcategory. Right-shifted curves correspond to higher-impact tiers; overlap indicates continuous variation. Cluster IDs are arbitrary and mapped to tiers post hoc by mean impact.", fig.width=13, fig.height=7}
ecdf_plot_all
```
...
AIRESS faculty can now propose manual updates to this list, either by suggesting adding or subtracting journals or altering a journals ranking.

Faculty may propose the inclusion of journals that do not appear in the current Scopus-based list or that do not meet the quantitative criteria used in this analysis. Such journals may be designated as off-list (or hors catégorie), reflecting their relevance for particular research areas, regional contexts, languages, or scholarly traditions that are not well captured by standard bibliometric indicators. Off-list journals are acknowledged as legitimate and potentially valuable publication venues; however, they are not evaluated or tiered using the PCA- and clustering-based framework described in this report. As a result, their inclusion does not imply equivalence with journals appearing in the A/B/C tiers, and they should be interpreted separately in evaluation, mentoring, and reporting contexts. This provision is intended to preserve disciplinary flexibility and intellectual diversity while maintaining transparency about which journals are evaluated using standardized quantitative criteria and which are recognized through faculty judgment.
