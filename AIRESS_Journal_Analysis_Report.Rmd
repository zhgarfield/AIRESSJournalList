...
```{r index-hist, fig.cap="The Impact Index (0–100) is derived from the first principal component (PC1) of CiteScore, SNIP, and SJR. Y-axes are scaled independently within each subcategory to emphasize the internal structure of impact distributions rather than differences in the number of journals across fields. The distribution is highly right-skewed, with most journals concentrated at the lower end of the impact spectrum and a long tail of increasingly influential outlets. This pattern reflects the typical unevenness of citation-based metrics, where a small subset of journals capture disproportionately high scholarly visibility and influence.", fig.width=13}
...
```{r clusterplot, fig.cap="K-means cluster centers by subcategory. The figure shows the mean Impact Index for each cluster within disciplinary subcategories. Clusters are ordered post hoc by mean impact, and tier labels (Excellent, Preferred, Acceptable) reflect relative standing rather than the numeric cluster identifiers produced by k-means.", fig.width=12}
...
```{r ecdfplot, fig.cap="ECDFs of the Impact Index by cluster and subcategory. Right-shifted curves correspond to higher-impact tiers; overlap indicates continuous variation. Cluster IDs are arbitrary and mapped to tiers post hoc by mean impact.", fig.width=13}
ecdf_plot_all
```
